...
public class GroupCreationTests extends TestBase {
    ...
    public Iterator<Object[]> validGroupsFromJson() throws IOException {
        ...
        Gson gson = new Gson();
        List<GroupData> groups = gson.fromJson(json, new TypeToken<List<GroupData>>() {}.getType()); //List<GroupData
        return  groups.stream().map((g) -> new Object[] {g}).collect(Collectors.toList()).iterator();
    }
    ...
    public void testGroupCreation1() {
        app.goTo().groupPage();
        Groups before = app.group().all();
        GroupData group = new GroupData().withName("Name1'").withFooter("foter1").withHeader("header1");
        app.group().create(group);
